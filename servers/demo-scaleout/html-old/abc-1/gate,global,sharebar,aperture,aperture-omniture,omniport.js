...
/** 
* @filepath: gate,global,sharebar,aperture,aperture-omniture,omniport
* @created: Tue, 26 Nov 13 12:59:30 -0800
* @serveraddr: 10.220.179.12
* @remoteaddr: 10.90.179.47
* @bu: abccom
...
var abcdm=abcdm||{};abcdm.abccom=abcdm.abccom||{};abcdm.abccom.Hook=abcdm.abccom.Hook||function(){};abcdm.abccom.Gate=abcdm.abccom.Gate||function(){};abcdm.abccom.debugLog=abcdm.abccom.debugLog||function(bug){return function(){};};(function($){abcdm.abccom.Gate.authUrl='/service/gate-auth';abcdm.abccom.Gate.contentId=null;abcdm.abccom.Gate.gateDiv=null;abcdm.abccom.Gate.gateType=null;abcdm.abccom.Gate.init=function(){if($('.gate').length){abcdm.abccom.Gate.contentId=$('.gate').attr('id').replace('gate_',''),abcdm.abccom.Gate.gateDiv='#gate_'+abcdm.abccom.Gate.contentId;if($(abcdm.abccom.Gate.gateDiv+' .challenge').length){abcdm.abccom.Gate.gateType='challenge';abcdm.abccom.Gate.checkLoggedIn();}else{$(abcdm.abccom.Gate.gateDiv).show();}}};abcdm.abccom.Gate.checkLoggedIn=function(){if(abcdm.abccom.Utils.userCheck('sdlt')!=='loggedout'){abcdm.abccom.Gate.checkEligibility();}else{$(abcdm.abccom.Gate.gateDiv).show();abcdm.abccom.Hook('loginComplete',{'hName':'checkEligibilityAfterLogin','hook':abcdm.abccom.Gate.checkEligibility});abcdm.abccom.Hook('regComplete',{'hName':'checkEligibilityAfterReg','hook':abcdm.abccom.Gate.checkEligibility});}};abcdm.abccom.Gate.checkEligibility=function(){var show='';$(abcdm.abccom.Gate.gateDiv+' .verify').show();if(typeof location.pathname.split('/')[2]==="string"&&$.inArray(location.pathname.split('/')[1],['shows','watch'])>-1){show=location.pathname.split('/')[2];}
$.post(abcdm.abccom.Gate.authUrl,'id='+abcdm.abccom.Gate.contentId+'&show='+show,function(response){if(response&&response.html){$(abcdm.abccom.Gate.gateDiv+' .verify').fadeOut('slow');setTimeout(function(){$(abcdm.abccom.Gate.gateDiv).removeClass('gate');$(abcdm.abccom.Gate.gateDiv).html(response.html).hide().fadeIn('slow');},1000);}else{$(abcdm.abccom.Gate.gateDiv+' .verify').html('Error validating access').fadeOut('slow');}},'json');};$(document).ready(function(){abcdm.abccom.Gate.init();});}(jQuery));
