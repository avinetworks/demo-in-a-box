        ...
        for a in args.metrics:
            if a.lower() not in endpoint_types:
                ...
                with open(os.path.join(fdir,'datadog.json')) as dd:
                    ...
                    endpoint_list.append(endpoint_info)
            elif a.lower() == 'prometheus':
                endpoint_info = {}
                endpoint_info['type'] = 'prometheus'
                endpoint_list.append(endpoint_info)
            elif a.lower() == 'dynatrace':
                with open(os.path.join(fdir,'dynatrace.json')) as dt:
                    endpoint_info = json.load(dt)['dynatrace']
                    endpoint_info['type'] = 'dynatrace'
                    endpoint_list.append(endpoint_info)
